...
def experiments(trials_vec):
    ...
    P_god_correcting_kz_errors = []
    for k in range(N):
        pkaux = comb(N-1, k) * ((cos(B))**2 + (nx*sin(B))**2)**(N-1-k) * (nz*sin(B))**(2*k)

        P_god_correcting_kz_errors.append( pkaux ) #Pk replace!

    p_aux_kz_errors            = P_god_correcting_kz_errors[0:N-1]
    p_aux_kz_errors            = np.array(p_aux_kz_errors)

    p_aux_kz_errors[np.abs(p_aux_kz_errors) < remove_prob_below] = 0
    p_aux_kz_errors            = list(p_aux_kz_errors)

    p_aux_kz_errors.append(np.abs(1.0 - sum(p_aux_kz_errors)))
    P_god_correcting_kz_errors = p_aux_kz_errors
    ...
    for iteration in range(itmax):
        ...
        ## Laboratory:
        ##---------------------------------------------------------------------------##
        random_data_kz_errors = list(np.random.choice(N, M, p = P_god_correcting_kz_errors) )
        ...
        counting_errors      = []
        for idx in range(N):
            counting_errors.append(random_data_kz_errors.count(idx))
        ...
        for Bx in Bx_vec:
            ...
            for Bz in Bz_vec:
                ...
                pk_prod_aux = []
                for k in range(N): 
                    pk_prod_aux.append((comb(N-1, k) *                                       ((cos(B))**2 + (nx*sin(B))**2)**(N-1-k) *                                        (nz*sin(B))**(2*k))**counting_errors[k] )
        ...
        for k in range(N):
            ...
            ## Laboratory:
            ##---------------------------------------------------------------------------##
            random_data_kz_errors      = list(np.random.choice(2, counting_errors[k], p = P_god) )
            ...
            for Beff in Beff_vec:
                likelihood_Beff_aux.append( ((cos(0.5 * 2 * Beff * (N - 1 - k)))**2)**counting_errors[0] *                                             ((sin(0.5 * 2 * Beff * (N - 1 - k)))**2)**counting_errors[1]  )
    ...
    max_value  = np.array(np.multiply(G, posteriorz)).max()
    max_coords = np.unravel_index(np.array(np.multiply(G, posteriorz)).argmax(),                                   np.array(np.multiply(G, posteriorz)).shape)
...
Beff_vec  = np.linspace(0.95, 1.05, 1000)
...
for N in N_vec:
    ...
    for idx in idx_vec:
        ...
        filename = f"Estimation_Z_errors_{idx}_N={N}.npz"
    ...
    plt.savefig(save_data +                         "Z_Scattered_QEC_Sensor_N="+str(N)+".pdf",                 bbox_inches='tight')
    plt.close()
